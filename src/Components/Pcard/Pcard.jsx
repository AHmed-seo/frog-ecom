...
const {addToCart} = useContext(CartContext)
  ...
  <Link to={`/details/${_id}`}>
    <div>
        <img className="h-[400px] w-full p-8 rounded-t-lg" src={imageCover} alt="product image" />
    </div>
    <div className="px-5 pb-5">
        <div>
            <h5 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{title}</h5>
        </div>
            <p className=" tracking-tight text-gray-900 dark:text-white">{description.slice(0,100)+ '...'}</p>
    ...
    <button onClick={()=>{addToCart(_id)}} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mb-5 ">Add to cart</button>
</div>
